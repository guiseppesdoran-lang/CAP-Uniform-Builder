...
(function () {
  ...
  const RACK = { CENTER_X: 440, BOTTOM_Y: 560, OFFSET_AFTER_9: true, OFFSET_RIGHT_PX: Math.round(RIB.W / 2) };
  ...
  // Standard 3-wide layout; bottom row Y is locked, rows touch (no vertical gap)
  function computeStandardLayout(items) {
    const rows = chunkRows(items, 3); // top→bottom
    const layout = [];
    let y = RACK.BOTTOM_Y;
  ...
  // Offset mode after 9 ribbons: 2-wide rows offset right; single rows centered over the pair below
  function computeOffsetLayout(items) {
    if (items.length <= 9) return computeStandardLayout(items);
    const rows = [];
    for (let i = 0; i < items.length; i += 2) rows.push(items.slice(i, i + 2)); // top→bottom
    const layout = [];
    let y = RACK.BOTTOM_Y;
    for (let r = rows.length - 1; r >= 0; r--) {
      const row = rows[r];
  ...
  // Over-rack badges sit 25px above the highest ribbon row, centered on the top-center ribbon
  function getOverRackBadgeAnchor(layout) {
    const { topY, topCenterX } = getTopRowAnchor(layout);
    if (topY == null) return { x: RACK.CENTER_X, y: RACK.BOTTOM_Y - RIB.H - 25 }; // estimate when no ribbons
