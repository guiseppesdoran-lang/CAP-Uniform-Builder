...
for(const award of awards){
  ...
  const canonicalId=core.canonicalAwardKey(award);
  const precedenceOverride=overrides.precedence?.[award.id] || overrides.precedence?.[canonicalId];
  const deviceOverride=overrides.devices?.[award.id] || overrides.devices?.[canonicalId];
