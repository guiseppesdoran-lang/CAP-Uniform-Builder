# U.S. Military Complete Catalog Audit

Generated: 2026-08-30T05:19:55.184Z

> This audit measures implemented, locally renderable data. It does not treat discovery links as authorization and does not count missing or unverified artwork as complete.

## Coverage

- Source discovery records: 183
- Official-source addition records: 6
- Wearable canonical awards after filtering: 176
- Rejected navigation/rank records: 6
- Military badge records: 166
- Explicit badge variants: 339
- Total badge configurations: 401
- Badges with verified precedence: 5
- Badges with verified placement: 132
- Badge artwork — AVAILABLE: 1054; NOT_APPLICABLE: 1; MISSING_ASSET: 107; UNVERIFIED: 0
- CAP metal/embroidered badge pairs: 25
- CAP paired badge artwork — AVAILABLE: 50; NOT_APPLICABLE: 0; MISSING_ASSET: 0; UNVERIFIED: 0
- Device definitions: 16
- Awards with at least one officially verified precedence entry: 142
- Awards with an explicit service device rule: 26
- Official precedence-table IDs missing from the canonical catalog: 4
- Available records with broken local paths: 0

## Award representation status

- Ribbon — AVAILABLE: 175; NOT_APPLICABLE: 0; MISSING_ASSET: 1; UNVERIFIED: 0
- Miniature medal — AVAILABLE: 80; NOT_APPLICABLE: 73; MISSING_ASSET: 23; UNVERIFIED: 0
- Full-size medal — AVAILABLE: 83; NOT_APPLICABLE: 72; MISSING_ASSET: 21; UNVERIFIED: 0

## Service discovery coverage

- ARMY: 76 canonical records containing that source-service tag
- MARINE_CORPS: 80 canonical records containing that source-service tag
- NAVY: 85 canonical records containing that source-service tag
- AIR_FORCE: 88 canonical records containing that source-service tag
- SPACE_FORCE: 8 canonical records containing that source-service tag
- COAST_GUARD: 89 canonical records containing that source-service tag

## Rejected non-award records

- `us_air_force_rank_cnc_and_laser_graphics_wood_working` — US Air Force Rank Free CNC and Laser Military Graphics and Designs,Free Military Graphics and Designs (RANK)
- `us_army_rank_cnc_and_laser_graphics_wood_working` — US Army Rank Free CNC and Laser Military Graphics and Designs Free Military Graphics and Designs (RANK)
- `us_coast_guard_rank_cnc_and_laser_graphics_wood_working` — US Coast Guard Free CNC and Laser Military Graphics and Designs Free Military Graphics and Designs (RANK)
- `us_marine_corps_rank_cnc_and_laser_graphics_wood_working` — Free Military Graphics and Designs (RANK)
- `us_navy_rank_cnc_and_laser_graphics_wood_working` — Free Military Graphics and Designs (RANK)
- `us_space_force_rank_cnc_and_laser_graphics_wood_working` — US Space Force Free CNC and Laser Military Graphics and Designs (RANK)

## Official table records absent from the catalog

- AIR_FORCE: `air_and_space_nuclear_deterrence_operations_service_medal`
- AIR_FORCE: `developmental_special_duty_ribbon`
- SPACE_FORCE: `air_and_space_nuclear_deterrence_operations_service_medal`
- SPACE_FORCE: `developmental_special_duty_ribbon`

## Interpretation

- Service counts are discovery coverage, not proof that every award is currently authorized for wear by that service.
- The military badge catalog is not complete until official-source records and approved local artwork are added.
- Medal gaps remain explicit. No generic medal pendant or ribbon-only substitute is accepted.
